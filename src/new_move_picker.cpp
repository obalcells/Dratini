...
bool NewMovePicker::bad_capture(Move move) {
...
void NewMovePicker::score_captures() {
    for(int i = 0; i < moves.size(); i++) {
        ...
        // scores.push_back(mvvlva(move))
    }
    assert(scores.size() == moves.size());
}

void NewMovePicker::score_quiet(int quiet_start) {
    for(int i = quiet_start; i < moves.size(); i++) {
        scores.push_back(thread->quiet_history[board->side][get_from(moves[i])][get_to(moves[i])]);
    }
    assert(scores.size() == moves.size());
}

// Static Exchange Evaluation
int NewMovePicker::see(Move move) const {
...
int NewMovePicker::next_lva(const uint64_t& attacker_mask, const bool attacker_side) const {
