...
#define PENALTY_ISOLATED_PAWN -1
#define PENALTY_FRONTAL_ENEMY_PAWN  -1
#define PENALTY_DOUBLE_PAWN -1
#define REWARD_DISTANCE_TO_PROMOTION_PAWN 1 
#define PENALTY_DISTANCE_FROM_START_PAWN -1
#define PENALTY_EMPTY_FILE_KING -1 // probably should be different for endgame
#define PENALTY_EMPTY_FILE_ENDGAME_KING -1 // yep
#define PENALTY_DISTANCE_TO_PAWN_KING -1
#define PENALTY_FRONTAL_PAWN_KING -1
#define PENALTY_FRIENDLY_BLOCKING_PIECE 0
#define REWARD_EMPTY_SQUARE_NON_SLIDING 1 // maybe it should be 0 and friendly +10?
#define REWARD_ENEMY_SQUARE_NON_SLIDING 1 
#define REWARD_EMPTY_SQUARE_SLIDING 1 
#define REWARD_ENEMY_SQUARE_SLIDING 1 
...
int eval() {
  ...
  for(int pos = 0; pos < 64; pos++) if(piece[pos] != EMPTY) {
    if(piece[pos] == PAWN) {
      if(color[pos] == WHITE && pawn_ranks[WHITE][col(pos)] == 0)
        pawn_ranks[WHITE][col(pos)] = row(pos);
      else if(color[pos] == BLACK)
        pawn_ranks[BLACK][col(pos)] = row(pos);
    }
    material[color[pos]] += piece_value[piece[pos]];
  }

  if(material[WHITE] + material[BLACK] <= 4500) {
    late_game = true;
  }

  int white_pawn_static = 0;
  int black_pawn_static = 0;
  ...
  for(int pos = 0; pos < 64; pos++) if(piece[pos] != EMPTY) {
    switch(piece[pos]) {
      case PAWN:
        if(color[pos] == WHITE) {
          white_pawn_static += eval_white_pawn(pos, late_game);
          white_pawn_static += pawn_pcsq[pos];
        } else {
          black_pawn_static += eval_black_pawn(pos, late_game);
          black_pawn_static += pawn_pcsq[flip(pos)];
        }
        break;
      case KING:
        material[color[pos]] += eval_king(pos, late_game);
        if(late_game)
          material[color[pos]] += king_endgame_pcsq[color[pos] == WHITE ? pos : flip(pos)];
        else
          material[color[pos]] += king_pcsq[color[pos] == WHITE ? pos : flip(pos)];
        break;
      case QUEEN:
  ...
  material[WHITE] += white_pawn_static;
  material[BLACK] += black_pawn_static;
  // assert(white_pawn_static - black_pawn_static == 0);
  
  std::cout << "******* START **********" << endl;
  print_board();
  std::cout << "Eval of the state is: " << material[WHITE] - material[BLACK] << endl;
  std::cout << "*******  END  **********" << endl;
...
int eval_black_pawn(int pos, bool late_game = false) {
  ...
  if(late_game) score += row(flip(pos)) * REWARD_DISTANCE_TO_PROMOTION_PAWN;
  else score += (7 - row(flip(pos))) * PENALTY_DISTANCE_FROM_START_PAWN;
